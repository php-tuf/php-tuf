...
class GuzzleLoader implements LoaderInterface
{
    ...
    public function load(string $locator, int $maxBytes): StreamInterface
    {
        ...
        $onProgress = function (int $expectedBytes, int $downloadedBytes) use ($locator, $maxBytes) {
        ...
        try {
            return $this->client->request('GET', $locator, $options)->getBody();
        } catch (ClientException $e) {
            if ($e->getCode() === 404) {
                throw new RepoFileNotFound("$locator not found", $e->getCode(), $e);
            } else {
