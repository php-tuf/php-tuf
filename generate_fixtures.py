...
class TUFTestFixtureBase:
    ...
    def __init__(self):
        ...
        self.repository = rt.create_new_repository(self.tufrepo_dir, type(self).__name__)
    ...
    def write_and_import_keypair(self, name_dst):
        ...
        pathpriv_src = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'fixture_keys', '{}_key'.format(self.next_keypair_index))
    ...
    def _initialize_basic_roles(self):
        ...
        (self.public_targets_key, private_targets_key) = self.write_and_import_keypair('targets')
        (self.public_snapshots_key, private_snapshots_key) = self.write_and_import_keypair('snapshot')
        (self.public_timestamps_key, private_timestamps_key) = self.write_and_import_keypair('timestamp')
        ...
        self.repository.snapshot.add_verification_key(self.public_snapshots_key)
        self.repository.snapshot.load_signing_key(private_snapshots_key)
        self.repository.timestamp.add_verification_key(self.public_timestamps_key)
    ...
    def write_and_publish_repository(self, regenerate_client=True):
        ...
        if regenerate_client:
            ...
            rt.create_tuf_client_directory(self.tufrepo_dir, client_tufrepo_dir)
...
class TUFTestFixtureDelegated(TUFTestFixtureSimple):
     def __init__(self):
        ...
        (public_targets_key_2, private_targets_key_2) = self.write_and_import_keypair('targets2')
        (public_snapshots_key_2, private_snapshots_key_2) = self.write_and_import_keypair('snapshot2')
